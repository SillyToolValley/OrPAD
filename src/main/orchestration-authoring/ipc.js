...
function authoringAgentPrompt({
  ...
  const lines = [
    ...
    '   k. Does the task target an external repository or assets NOT already present in the workspace (a github/gitlab URL, "clone X", "test repo Y")? If yes, add an `orpad.provision` node BEFORE the work queue. Workers run in a sliced sandbox overlay and CANNOT clone or fetch; without provisioning the run will block on a missing checkout.',
    '   l. Do any pre-queue gates demand evidence that only WORKERS can produce (something "documented", "recorded", "confirmed", "verified")? Pre-queue gates may only assert facts that exist before workers run (provision steps completed, probes executed, queue seeded) — completed-work criteria belong on the post-worker verification gate, otherwise the gate blocks the very workers that would satisfy it.',
    ...
    '- The 18 valid node types are: orpad.entry, orpad.exit, orpad.selector, orpad.gate, orpad.barrier, orpad.context, orpad.probe, orpad.rule, orpad.skill, orpad.graph, orpad.tree, orpad.workQueue, orpad.triage, orpad.dispatcher, orpad.workerLoop, orpad.patchReview, orpad.artifactContract, orpad.provision.',
    '- `orpad.provision` acquires the environment deterministically (machine-side, no LLM) before worker fan-out. config: { steps: [{ kind: "git-clone", repo: "https://...", targetDir: "relative/dir", ref?: "branch" }, { kind: "install", tool: "npm"|"pnpm"|"yarn", dir: "relative/dir" }], onFail?: "block"(default)|"warn" }. targetDir/dir must be workspace-relative (never absolute, never under .orpad). Downstream work items must list the provisioned directory in sourceOfTruthTargets so workers can read the tree.',
