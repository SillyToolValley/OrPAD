...
- `machine-execute-run-step` is limited to a local deterministic `run.machineHarness` fixture in
  ...
  WorkerLoop nodes, executes Machine-owned support nodes such as WorkQueue/Gate/Barrier/
  ArtifactContract lifecycle markers, ingests the harness candidate, claims via the dispatcher,
  runs one WorkerLoop step, finalizes run status from Machine queue inventory, and constructs the
  exact CLI overlay command itself. The renderer cannot pass an arbitrary command, args, cwd, or
  dangerous Codex bypass flag through this channel.
