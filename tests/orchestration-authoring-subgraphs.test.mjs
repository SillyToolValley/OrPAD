...
test('generator emits a placeholder sub-graph when the spec forgot a subgraphs entry', async (t) => {
  ...
  // Placeholder is recognizable: a TODO context node sits between entry and exit.
  const todo = stub.graph.nodes.find(n => n.id === 'todo' || /todo/i.test(n.label || ''));
  assert.ok(todo, 'placeholder sub-graph should contain a TODO node');
  assert.match(String(todo.config?.summary || todo.label || ''), /Placeholder|TODO/i);
});
...
test('generator auto-fills missing graphRef/treeRef and preserves parent hints', async (t) => {
  ...
  const subgraph = JSON.parse(await fs.readFile(subgraphPath, 'utf-8'));
  assert.equal(subgraph.kind, 'orpad.graph');
  // (c) parent hints are preserved inside the placeholder's TODO node
  const todo = subgraph.graph.nodes.find(n => n.id === 'todo');
  assert.ok(todo, 'placeholder should contain a TODO node');
  const todoSummary = String(todo.config?.summary || '');
  assert.match(todoSummary, /Plan Unit1 repairs/);
  assert.match(todoSummary, /Beginner: thread vs process/);
  assert.match(todoSummary, /Unit1 only/);
...
test('generator defaults orpad.graph executionMode to inline and preserves explicit overrides', async (t) => {
  ...
  const authoringSpec = {
    ...
    subgraphs: [
      { ref: 'graphs/plan-default.or-graph', graph: { id: 'pd', label: 'pd', start: 'e', nodes: [{ id: 'e', type: 'orpad.entry', label: 'e' }], transitions: [] } },
      { ref: 'graphs/plan-module.or-graph', graph: { id: 'pm', label: 'pm', start: 'e', nodes: [{ id: 'e', type: 'orpad.entry', label: 'e' }], transitions: [] } },
    ],
  };
...
test('generator rewrites duplicate orpad.graph refs so each parent gets a unique invocation identity', async (t) => {
  ...
  const authoringSpec = {
    ...
    subgraphs: [
      { ref: 'graphs/shared.or-graph', graph: { id: 'shared', label: 'shared', start: 'e', nodes: [{ id: 'e', type: 'orpad.entry', label: 'e' }], transitions: [] } },
    ],
  };
...
test('safeRefPath rejects refs that climb out of the pipeline directory', async (t) => {
  ...
  const expectedInsideDir = path.join(result.pipelineDir, 'etc');
  await assert.rejects(fs.stat(expectedInsideDir));
});
